...
**用户故事：** 作为产品运营人员，我希望了解用户在前端的行为路径，以便优化产品体验和驱动数据决策。

#### 验收标准

1. THE Frontend SHALL 自动采集以下埋点事件：

| 事件名 | 触发时机 | 参数 |
|--------|----------|------|
| `page_view` | 路由切换 | `page_name`, `timestamp` |
| `message_sent` | 用户发送消息 | `thread_id`, `has_media`, `media_type` |
| `message_received` | AI 回复完成（`[DONE]`） | `thread_id`, `token_count`, `ttft_ms` |
| `file_upload` | 文件上传完成 | `file_type`, `file_size_kb`, `duration_ms` |
| `hitl_requested` | 触发人工介入 | `thread_id`, `reason`, `unresolved_turns` |
| `config_changed` | 租户配置修改 | `config_type` |
| `sandbox_run` | 沙盒测试执行 | `test_case_count`, `overall_passed` |
| `error_occurred` | API 错误或网络异常 | `status_code`, `endpoint`, `error_type` |

2. THE Frontend SHALL 在非生产环境（development / staging）禁用埋点上报，避免污染数据。
3. THE Frontend SHALL 在用户同意隐私政策后才上报埋点数据（遵循 GDPR / 个人信息保护法）。
4. THE Frontend SHALL 通过统一的 `analytics.track(event_name, params)` 接口上报事件，底层可对接多种分析平台（如 PostHog、Mixpanel 或自建）。

---

### 需求 14：C 端 API Key 配置与 Chat_Widget 嵌入

**用户故事：** 作为企业租户的技术人员，我希望能够快速将 Chat_Widget 嵌入到企业网站中，并通过 API Key 完成鉴权配置。

#### 验收标准

1. THE Chat_Widget SHALL 支持通过 `<script>` 标签嵌入，配置项通过 `data-*` 属性或全局配置对象传入：
```html
...
```
2. THE Chat_Widget SHALL 支持主题定制：`light` / `dark` 模式，以及通过 CSS 变量覆盖品牌色（`--ce-primary-color`、`--ce-bg-color`）和 Logo URL。
3. THE Admin_Dashboard SHALL 在配置中心提供"嵌入代码生成器"，Admin_User 选择配置后自动生成可复制的 `<script>` 代码片段。
4. THE Chat_Widget SHALL 在 API Key 无效时（后端返回 401）展示"配置错误，请联系管理员"的提示，不展示聊天界面。


---

### 需求 15：用户反馈与 AI 回答质量闭环

**用户故事：** 作为 C 端用户，我希望能够对 AI 的回答进行评价（点赞/点踩），以帮助系统持续改进回答质量。

#### 验收标准

1. THE Chat_Widget SHALL 在每条 AI 回复的 Message_Bubble 底部展示 Thumb Up / Thumb Down 反馈按钮，用户点击后高亮选中状态。
2. WHEN End_User 点击 Thumb Down 时，THE Chat_Widget SHALL 展示可选的反馈原因（"回答不准确"、"没有回答我的问题"、"回答太慢"、"其他"），并支持输入自由文本补充。
3. THE Chat_Widget SHALL 将反馈数据（thread_id、message_id、rating、reason）上报至后端，作为 RAG 系统自我进化的核心数据源。
4. THE Metrics_Dashboard SHALL 在数据看板中展示用户满意度指标（Thumb Up 率），并支持按时间范围筛选。

---

### 补充说明（来自专家评审）

> **协议平滑切换**（需求 1.5 补充）：当从 SSE 切换到 WebSocket 时，前端 SHALL 确保存量消息不重闪（Flash），输入框状态无缝衔接。切换过程中 SHALL 保持消息列表的滚动位置不变。

> **虚拟滚动实现**（需求 1.9 补充）：推荐使用 `react-virtuoso` 或 `@tanstack/react-virtual`，开启动态高度测量模式，结合 `ResizeObserver` 监听气泡高度变化（AI 消息含 Markdown 代码块、图片等动态内容）。

> **语音波形指示器**（需求 2.4 补充）：在 ASR 阶段（`stage: asr`），UI 除展示"语音识别中"文字外，SHALL 加入波形指示器（Waveform visualizer）动画，反馈系统正在处理，降低用户感知等待时间。

> **图片粘贴上传**（需求 2 补充）：THE Media_Uploader SHALL 支持图片粘贴（Ctrl+V / Cmd+V）上传，提升操作效率。

> **沙盒对比视图**（需求 9 补充）：THE Sandbox_Runner SHALL 支持"对比视图"，左侧展示旧配置的 Ragas 得分，右侧展示新配置得分，点击差异项可对比同一测试用例的回答差异。

> **iOS 视口兼容**（需求 2.8 补充）：移动端 SHALL 使用 `window.visualViewport` API 进行精确高度计算，解决 iOS Safari 中 `100vh` 和软键盘弹出的兼容性问题。

> **性能 SLA**（全局补充）：首屏加载时间（FCP）SHALL < 1.5 秒（桌面端）/ < 3 秒（移动端 3G）；交互响应时间（INP）SHALL < 200ms。
