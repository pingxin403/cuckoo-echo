...
- [x] 30. Docker Rebuild + SSE 逐 token 验证
  ...
  - [x] 30.2 Rebuild knowledge-pipeline Docker 镜像并修复 Docling 依赖
...
- 阶段五（Task 26-29）：文档更新 + 测试补全 + E2E 视觉检查
- 阶段六（Task 30-34）：Docker 端到端验证 + 后端集成测试
- 阶段七（Task 35-39）：可观测性 + 负载测试 + 生产部署准备
- 阶段八（Task 40-43）：安全加固 + 性能优化 + 部署加固
